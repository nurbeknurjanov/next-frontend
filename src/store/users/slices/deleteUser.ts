...
const selector = {
  ...
  isFetching: (state: RootStateType) => state.users.deleteUser.isFetching,
  data: (state: RootStateType) => state.users.deleteUser.data,
  error: (state: RootStateType) => state.users.deleteUser.error,
};

const initialState: DeleteUserStateType = {
  ...getInitialResponseState(),
  error: null,
  data: null,
};

const SLICE_NAME = '@users/deleteUser';
...
export const deleteUserObject: DeleteUserType = {
  actions,
  thunk: {
